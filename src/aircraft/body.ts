...
export function calculateObjectMomOfInertia(simBody: AircraftBody, index: number) {
    const correctedPos: Vec3[] = simBody.components.map(component => 
        component.centroid.sub(simBody.centerOfGravity));

    for (let i = 0; i < simBody.components.length; i++) {
        var mass = simBody.components[i].mass;

        var Ixx = mass * (Math.pow(correctedPos[i].y(),2) + Math.pow(correctedPos[i].z(),2));
        var Iyy = mass * (Math.pow(correctedPos[i].x(),2) + Math.pow(correctedPos[i].z(),2));
        var Izz = mass * (Math.pow(correctedPos[i].x(),2) + Math.pow(correctedPos[i].y(),2));
        var Iyz = mass * correctedPos[i].y() * correctedPos[i].z();

        simBody.components[i].momOfInertia.setIxx(Ixx);
        simBody.components[i].momOfInertia.setIyy(Iyy);
        simBody.components[i].momOfInertia.setIzz(Izz);
        simBody.components[i].momOfInertia.setIyz(Iyz);
    }
}

export function calculateAircraftMomOfInertia(simBody: AircraftBody) {
    for (let i = 0; i < simBody.components.length; i++) {
        simBody.momOfInertia.add(simBody.components[i].momOfInertia);
    }
}